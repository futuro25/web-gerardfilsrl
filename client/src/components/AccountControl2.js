...
/**
 * Saldo del banco seleccionado. A diferencia de Control (que suma todas las
 * cuentas propias), acá el saldo, sus totales del mes y sus cheques son
 * exclusivos del banco elegido en el selector: cada banco tiene su propio saldo.
 */
function SelectedBankBalanceCard({ summary, bank }) {
  const total = summary?.balanceWithoutCheques ?? 0;
  const withCheques = summary?.balanceWithCheques ?? 0;
  ...
  return (
    <div className="bg-white border rounded-lg p-4 shadow-sm">
      ...
      <div className="mt-2 flex items-center justify-between gap-2 text-xs">
        <span className="text-slate-500">Con cheques a cobrar/pagar</span>
        <span
          className={utils.cn(
            "font-medium tabular-nums shrink-0",
            withCheques >= 0 ? "text-slate-700" : "text-red-600"
          )}
        >
          {utils.formatAmount(withCheques)}
        </span>
      </div>
...
/** Cuenta propia con la que se opera en este módulo: cada banco tiene su propio saldo. */
const CONTROL2_BANK_STORAGE_KEY = "control2_selected_bank";

export default function AccountControl2() {
  const navigate = useNavigate();
  const queryClient = useQueryClient();

  const [selectedBank, setSelectedBank] = useState(() => {
    try {
      const saved = localStorage.getItem(CONTROL2_BANK_STORAGE_KEY);
      return utils.getOwnBanks().includes(saved) ? saved : "GALICIA";
    } catch (e) {
      return "GALICIA";
    }
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem(CONTROL2_BANK_STORAGE_KEY, selectedBank);
    } catch (e) {
  ...
  const summaryParams = viewAll
    ? { bank: selectedBank }
    : { month: selectedMonth, year: selectedYear, bank: selectedBank };
  ...
  const { data: upcomingCheques } = useQuery({
    queryKey: queryUpcomingChequesKey({ bank: selectedBank }),
    queryFn: () => fetchUpcomingCheques(15, selectedBank),
  });
  ...
  return (
    <>
        ...
        <div className="flex gap-2 items-center justify-between text-xl font-bold text-center pl-2 w-full mr-2">
          ...
          <div className="flex items-center gap-3 ml-auto">
            <div className="flex items-center gap-2">
              <label
                htmlFor="control2-bank-selector"
                className="text-[11px] font-medium text-slate-500 uppercase tracking-wide whitespace-nowrap"
              >
                Banco
              </label>
              <select
                id="control2-bank-selector"
                className="border border-gray-200 rounded px-2 py-1.5 text-sm font-medium bg-white text-slate-800 focus:outline-none focus:border-slate-400"
                value={selectedBank}
                onChange={(e) => handleBankChange(e.target.value)}
                title="Banco con el que se opera: filtra movimientos, saldo y cheques"
              >
        ...
        {stage === "LIST" && (
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 my-4">
              ...
              <SelectedBankBalanceCard summary={summary} bank={selectedBank} />
            </div>
            ...
            <Dialog open={futureDialogOpen} onOpenChange={setFutureDialogOpen}>
              <DialogContent className="w-[95vw] max-w-3xl max-h-[85vh] overflow-hidden flex flex-col p-6 gap-3">
                ...
                <p className="text-xs text-slate-500">
                  ...
                  <strong>Suma todas las cuentas propias</strong>, no solo{" "}
                  {selectedBank}.
                </p>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
              ...
              {requiresPaymentMethod && (
                <PaymentOrderFields
                  ...
                  defaultBank={selectedMovement?.bank || selectedBank}
                  defaultPaymentDate={
      ...
      <ConfirmDialog
        ...
        description={
          ...
          (cancelOpConfirm.movement.payment_orders || []).filter(
            ...
            ? "La orden de pago se anulará. Las demás OP seguirán activas y el saldo se recalculará en Control 2 y en la cuenta corriente."
            : "La orden de pago se anulará y el movimiento volverá a pendiente de pago. El saldo se actualizará en Control 2 y en la cuenta corriente."
        }
      ...
      <ConfirmDialog
        ...
        description="El VEP pasará a estado pagado y dejará de mostrarse en los avisos de vencimiento de Control 2."
